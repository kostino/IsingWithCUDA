...
#define THREADDIM 4 //This is (int)sqrt(MAXMOMENTSPERTHREAD)

__constant__ double dev_w[25];
...
void apply_w(int * data,int * result, double * filter, int n){
	...
	for(int x=-2;x<BLOCKDIM*THREADDIM+2;x++){
		for(int y=-2;y<BLOCKDIM*THREADDIM+2;y++){
			G_sh[x+2][y+2]=
		}
	}
...
void ising( int *G, double *w, int k, int n){
	...
	int * dev_G;
	int * dev_res;
	if(cudaMalloc(&dev_G,n*n*sizeof(int))!=cudaSuccess||cudaMalloc(&dev_res,n*n*sizeof(int))!=cudaSuccess){
	...
	cudaMemcpyToSymbol(dev_w,w,25*sizeof(double));
